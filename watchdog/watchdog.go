...
//
// TODO(pires) respect threshold (default to 25)
// TODO(pires) making the decision to restart should rely on a trend,
...
func (w *watchdog) checkLocalNode(ctx context.Context) {
	...
	for {
		...
		case <-tick.C:
			...
			// Set span status.
			// TODO(pires) logically, this can't be nil but we should protect
			// ourselves.
			span.RecordError(err)
			span.SetStatus(otelcodes.Error, message)

			// Trigger state-machine event.
			// TODO(pires) differentiate between unhealthy and unreachable.
	...
	if err != nil {
		...
		if rpcErr, ok := err.(*soljsonrpc.RPCError); ok {
			...
			if rpcErr.Code == solanaErrNodeIsUnhealthy {
				// TODO(pires) check rpcErr.Data type in Solana's code and
				// respect it? For now, we just need numSlotsBehind so
				// map[string]int works fine.
				//
				// NOTE(ppires) same error code has different data structures:
